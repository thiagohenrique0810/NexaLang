...
def main(argv=None):
    ...
    parser.add_argument("--tensor-codec", action="append", dest="tensor_codecs", metavar="NAME=CODEC",
                        help="Store this matrix with q2, q3, q4, q8, f16 or f32; repeat per tensor")
    parser.add_argument("--matrix-codec", choices=("q2", "q3", "q4", "q8", "f16", "f32"),
            ...
            if args.precision_map is not None:
                ...
                if args.matrix_codec:
                    ...
                    for item in args.tensor_codecs:
                        ...
                        if not separator or codec not in ("q2", "q3", "q4", "q8", "f16", "f32") or name not in matrices:
                            parser.error(f"--tensor-codec expects NAME=q2|q3|q4|q8|f16|f32 for a matrix: {item}")
                        codecs[name] = codec
